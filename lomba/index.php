<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" href="foto/logo.jpg" type="image/x-icon">
    <title>Bem ITBA Dcc Kampus Kabupaten Lampung Utara</title>
    <link rel="stylesheet" href="styles.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" integrity="sha512-iecdLmaskl7CVkqkXNQ/ZH/XLlvWZOJyj7Yy7tcenmpD1ypASozpmT/E0iPtmFIB46ZmdtAc9eNBvH0H/ZpiBw==" crossorigin="anonymous" referrerpolicy="no-referrer" />
</head>
<body>
    <!-- Header -->
    <header>
        <div class="logo">
            <img src="foto/logo.jpg">
            <p>Bem ITBA Dcc Kampus Kabupaten Lampung Utara</p>
        </div>
        <nav>
            <ul>
                <li><a href="index.php">Beranda</a></li>
                <li><a href="tentang.php">Tentang Kami</a></li>
                <li><a href="layanan.php">Layanan</a></li>
                <li><a href="proyek.php">Proyek</a></li>
                <li><a href="kontak.php">Kontak</a></li>
                <li><a href="blog.php">Blog</a></li>
            </ul>
        </nav>
        <div class="menu-toggle" onclick="toggleMenu();">&#9776;</div>
    </header>

    <!-- Hero Section -->
    <section id="hero">
        <h1>Merdeka dengan Teknologi Transformasi Digital</h1>
        <p>Mewujudkan Indonesia yang Lebih Maju dengan Teknologi</p>
        <a href="#layanan" class="btn">Pelajari Lebih Lanjut</a>
    </section>

    <!-- Layanan Kami -->
    <section id="layanan">
        <h2>Layanan Kami</h2>
        <div class="layanan-container">
            <div class="layanan-item">
                <div class="blog-item">
                    <img src="transformasi_digital.png" alt="Transformasi Digital">
                    <h3>Transformasi Digital</h3>
                </div>
            </div>
            <div class="layanan-item">
                <div class="blog-item">
                    <img src="pengembangan_aplikasi.png" alt="Pengembangan Aplikasi">
                    <h3>Pengembangan Aplikasi</h3>
                </div>
            </div>
            <div class="layanan-item">
                <div class="blog-item">
                    <img src="keamanan_cyber.png" alt="Keamanan Cyber">
                    <h3>Keamanan Cyber</h3>
                </div>
            </div>
            <div class="layanan-item">
                <div class="blog-item">
                    <img src="konsultasi_teknologi.png" alt="Konsultasi Teknologi">
                    <h3>Konsultasi Teknologi</h3>
                </div>
            </div>
        </div>
    </section>

    <!-- Proyek dan Testimoni -->
    <section id="proyek">
        <h2>Proyek dan Testimoni</h2>
        <div class="proyek-container">
            <div class="proyek-item">
                <div class="blog-item">
                    <img src="proyek1.jpg" alt="Proyek 1">
                    <h3>Proyek 1</h3>
                    <p>Detail tentang proyek 1.</p>
                </div>
            </div>
            <div class="proyek-item">
                <div class="blog-item">
                    <img src="proyek2.jpg" alt="Proyek 2">
                    <h3>Proyek 2</h3>
                    <p>Detail tentang proyek 2.</p>
                </div>
            </div>
            <div class="proyek-item">
                <div class="blog-item">
                    <img src="proyek3.jpg" alt="Proyek 3">
                    <h3>Proyek 3</h3>
                    <p>Detail tentang proyek 3.</p>
                </div>
            </div>
        </div>
    </section>

    <!-- Footer -->
    <footer>
        <div class="kontak">
				<a href="index.php"><i class="fas fa-home" title="HOME"></i></a>
				<a href="kontak.php"><i class="fas fa-envelope" title="KONTAK"></i></a>
				<a style="margin-bottom: 15px;" href="https://api.whatsapp.com/send?phone=08974746874647&text=Saya%20membutuhkan%20bantuan%20🙏" target="_blank" rel="noopener noreferrer"><i class="fab fa-whatsapp" title="WHATSAPP"></i></a>
				<a style="margin-bottom: 15px;" href="http://instagram.com/_u/USERNAME/" target="_blank" rel="noopener noreferrer"><i class="fab fa-instagram" title="INSTAGRAM"></i></a>
        </div>
        <div class="alamat">
            <p>Alamat: Jl. Raya Lintas Utama Sumatera No.03, Candi MAS, Kec. Abung Sel., Kabupaten Lampung Utara, Lampung 34511</p>
        </div>
        <p>&copy; 2024 Bem ITBA Dcc Kampus Kabupaten Lampung Utara. All rights reserved.</p>
    </footer>

    <!-- Link to CSS and JavaScript -->
    <link rel="stylesheet" href="styles.css">
    <script src="scripts.js"></script>
</body>
</html>
